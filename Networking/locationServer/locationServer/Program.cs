...
namespace locationServer
{
    class Program
    {
        ...
        static public void match(string rawInput, Dictionary<string, string> dataStore, StreamWriter streamWrite)
        {
            ...
            if (Regex.IsMatch(rawInput, whoisEdit))
            {
            ...
            if (Regex.IsMatch(rawInput, http09Find))
            {
                foreach (Match info in Regex.Matches(rawInput, http09Find))
                {
                    ...
                    {
                        streamWrite.Write("HTTTP/0.9 200 OK\nContent-Type: text/plain\n\n" + record + "\n");
                    }
                }
            }
            if (Regex.IsMatch(rawInput, http09Edit))
            {
            ...
            if (Regex.IsMatch(rawInput, http10Find))
            {
                foreach(Match info in Regex.Matches(rawInput, http10Find))
                {
                    ...
                    {
                        streamWrite.Write("HTTTP/1.0 200 OK\nContent-Type: text/plain\n\n" + record + "\n");
                    }
                }
            }
            if (Regex.IsMatch(rawInput, http10Edit))
            {
            ...
            if (Regex.IsMatch(rawInput, http11Find))
            {
                foreach (Match info in Regex.Matches(rawInput, http11Find))
                {
                    ...
                    {
                        streamWrite.Write("HTTTP/1.1 200 OK\nContent-Type: text/plain\n\n" + record + "\n");
                    }
                }
            }
            if (Regex.IsMatch(rawInput, http11Edit))
            {
